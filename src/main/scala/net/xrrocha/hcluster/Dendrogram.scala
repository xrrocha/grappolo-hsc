...
case class Leaf(cluster: Cluster) extends Dendrogram {
  def collectClusters(accum: IndexedSeq[Cluster]): IndexedSeq[Cluster] = cluster +: accum

  def doCutAt(accum: IndexedSeq[Dendrogram], threshold: Similarity): IndexedSeq[Dendrogram] = this +: accum
}

case class Branch(similarity: Similarity, left: Dendrogram, right: Dendrogram) extends Dendrogram {
  def collectClusters(accum: IndexedSeq[Cluster]): IndexedSeq[Cluster] = right.collectClusters(left.collectClusters(accum))
...
object Dendrogram extends LazyLogging {
  ...
  def apply(size: Int, similarityMatrix: SimilarityMatrix): Dendrogram = {
    ...
    val seedMap: Map[Index, Dendrogram] = Map[Int, Dendrogram](seedPairs *)
